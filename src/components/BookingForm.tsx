...
import { FiCheck, FiChevronLeft, FiChevronRight } from "./icons";
import {
  availableSlots,
...
export default function BookingForm({
  ...
  compact?: boolean;
  /**
   * Контекст заявки, посчитанный не человеком (расчёт калькулятора, тариф).
   * Идёт в заявку вместе с комментарием, но в поле не показывается: править
   * его лиду незачем, а на созвон с готовыми цифрами приходить полезно.
   */
  contextNote?: string;
}) {
    ...
    try {
      const res = await fetch("/api/book-call", {
        ...
        body: JSON.stringify({
          ...
          interest,
          note: [contextNote, note.trim()].filter(Boolean).join("\n"),
          source,
          locale,
        }),
      });
    ...
    >
      ...
      >
        ...
        <div className={compact ? "" : "sm:border-l sm:border-white/[0.08] sm:pl-5"}>
          ...
          {selectedDay && slots.length === 0 && (
            <p className="mt-3 text-sm text-secondary">{t.noSlots}</p>
          )}
      ...
      <div className="mt-6 border-t border-white/[0.08] pt-5">
        ...
        <div className="mt-3">
          ...
          />
          {channel === "email" && (
            <p className="mt-1.5 text-xs text-secondary">{t.contactHintEmail}</p>
          )}
        </div>
