...
describe("resolveContact", () => {
  ...
  it("нормализует телеграм-ник и отвергает непохожее", () => {
    expect(resolveContact("telegram", "f1_owe", email)).toBe("@f1_owe");
    expect(resolveContact("telegram", "@f1_owe", email)).toBe("@f1_owe");
    expect(resolveContact("telegram", "ab", email)).toBeNull();
    expect(resolveContact("telegram", "+420773693263", email)).toBeNull();
  });

  it("чистит телефон от разделителей", () => {
    expect(resolveContact("phone", "+420 773 693-263", email)).toBe("+420773693263");
    expect(resolveContact("whatsapp", "(067) 123 45 67", email)).toBe("0671234567");
    expect(resolveContact("phone", "не скажу", email)).toBeNull();
  });

  it("для канала email подставляет email заявки, когда поле пустое", () => {
    expect(resolveContact("email", "", email)).toBe(email);
    expect(resolveContact("email", "other@example.com", email)).toBe(
      "other@example.com"
    );
    expect(resolveContact("email", "not-an-email", email)).toBeNull();
  });
});
