...
import {
  isValidContact,
  normalizeContact as canonicalContact,
  type ContactChannel,
} from "./contactChannel";
import { emailProblem } from "./emailCheck";

// Канал связи один на весь сайт: форма подписки и запись на созвон спрашивают
// одно и то же, и в таблицу должно попадать одно и то же. Определение и
// нормализация живут в contactChannel.ts, здесь — только реэкспорт, чтобы
// импорт «всё для брони из одного модуля» продолжал работать.
export {
  CONTACT_CHANNELS,
  isContactChannel,
  type ContactChannel,
...
/**
 * Контакт заявки на созвон: канон или null, если формат не похож на канал.
 * От обычной нормализации отличается двумя вещами — пустой email-контакт
 * заменяется основным адресом заявки, а неподходящий формат отбивается, а не
 * сохраняется как есть: без рабочего контакта созвон не состоится.
 */
export function resolveContact(
  ...
  return isValidContact(channel, raw) ? canonicalContact(channel, raw) : null;
}
